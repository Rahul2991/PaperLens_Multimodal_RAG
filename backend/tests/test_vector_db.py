...
def mock_qdrant_client():
    ...
    mock_client.collection_exists.return_value = (
        False  # Simulate new collection scenario
    )
    return mock_client
...
def test_qdrant_vdb_unreachable_server(qdrant_vdb, mock_qdrant_client, caplog):
    ...
    mock_qdrant_client.collection_exists.side_effect = grpc.RpcError(
...
def test_ingest_data(qdrant_vdb, mock_qdrant_client):
    ...
    mock_qdrant_client.upload_collection.assert_called_once_with(
        collection_name="test_collection",
        vectors=mock_embeddata.embeddings,
        payload=[{"context": context, "source": "test_source"} for context in mock_embeddata.contexts]
    )
